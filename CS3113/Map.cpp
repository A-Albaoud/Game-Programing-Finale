...
Map::Map(int mapColumns, int mapRows, unsigned int *levelData,
         ...
         std::unordered_set<unsigned int> walkableTiles) :
         mMapColumns {mapColumns}, mMapRows {mapRows},
         mLevelData {levelData}, mTextureAtlas {LoadTexture(textureFilePath)},
         mTileSize {tileSize}, mTextureColumns {textureColumns},
         mTextureRows {textureRows}, mWalkableTiles {walkableTiles},
...
int Map::getTileAt(Vector2 position) const
{
    ...
    int tileX = (int)floorf((position.x - mLeftBoundary) / mTileSize);
    int tileY = (int)floorf((position.y - mTopBoundary)  / mTileSize);

    if (tileX < 0 || tileX >= mMapColumns || tileY < 0 || tileY >= mMapRows)
        return -1;

    return (int)mLevelData[tileY * mMapColumns + tileX];
}

void Map::setTileAt(Vector2 position, unsigned int value)
{
    if (position.x < mLeftBoundary || position.x > mRightBoundary ||
        position.y < mTopBoundary  || position.y > mBottomBoundary)
        return;
    ...
    mLevelData[tileY * mMapColumns + tileX] = value;
}
...
bool Map::isSolidTileAt(Vector2 position, float *xOverlap, float *yOverlap)
{
    ...
    *yOverlap = 0.0f;
    ...
    if (mWalkableTiles.count(tile)) return false;
